...
from transformers import AutoConfig, AutoModelForCausalLM, DeepseekV3Config, DeepseekV3Model, DeepseekV3ForCausalLM
...
__all__ = [
    "LlavaDeepseekV3Config", "LlavaDeepseekV3Model", "LlavaDeepseekV3ForCausalLM",
]


class LlavaDeepseekV3Config(DeepseekV3Config):
    """Configuration class for llava_deepseek_r1 model."""
    model_type: str = "llava_deepseek_r1"


class LlavaDeepseekV3Model(LlavaMetaModel, DeepseekV3Model):
    """LlavaDeepseekV3Model model class."""
    config_class: LlavaDeepseekV3Config = LlavaDeepseekV3Config

    def __init__(self, config: DeepseekV3Config) -> None:
        """Initialize the LlavaDeepseekV3Model model.

        Args:
            config (DeepseekV3Config): Configuration for the LlavaDeepseekV3Model model.
        """
        super().__init__(
            config=config,
        )
...
class LlavaDeepseekV3ForCausalLM(DeepseekV3ForCausalLM, LlavaMetaForCausalLM):
    """LlavaDeepseekV3ForCausalLM model for causal language modeling."""
    config_class: LlavaDeepseekV3Config = LlavaDeepseekV3Config

    def __init__(self, config: DeepseekV3Config) -> None:
        """Initialize the LlavaDeepseekV3ForCausalLM model for causal language modeling.

        Args:
            config (DeepseekV3Config): Configuration for the LlavaDeepseekV3ForCausalLM model.
        """
        super().__init__(
            config=config,
        )
        self.model: LlavaDeepseekV3Model = LlavaDeepseekV3Model(config)
    ...
    def get_model(self) -> LlavaDeepseekV3Model:
...
AutoConfig.register("llava_deepseek_r1", LlavaDeepseekV3Config)
AutoModelForCausalLM.register(LlavaDeepseekV3Config, LlavaDeepseekV3ForCausalLM)
